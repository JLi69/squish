...
namespace assets {
	...
	void FontManager::importFromFile(nk_glfw *glfw, const char *path)
	{
		std::vector<impfile::Entry> entries = impfile::parseFile(path);
		...
		nk_glfw3_font_stash_begin(glfw, &fontatlas);
	...
	void FontManager::pushFont(nk_context *ctx, const std::string &fontname)
	{
		if(!fonts.count(fontname))
			return;

		nk_style_push_font(ctx, &fonts.at(fontname)->handle);
	}

	void FontManager::popFont(nk_context *ctx)
	{	
